...
describe("testInventorySummaryEmbed", () => {
  test("includes tier counts and doc link", () => {
    const embed = testInventorySummaryEmbed(samplePayload, "2026-07-03T00:00:00.000Z");
...
describe("buildTestInventoryEmbeds", () => {
  test("builds summary plus tier embeds without attachment", () => {
    const embeds = buildTestInventoryEmbeds(samplePayload, "2026-07-03T00:00:00.000Z");

    expect(embeds.length).toBeGreaterThan(1);
    expect(embeds[0]?.data.title).toBe("Room TBA test inventory");
    expect(embeds.some((e) => e.data.title?.startsWith("Unit tests (Bun)"))).toBe(true);
    expect(
      embeds.some((e) => e.data.description?.includes("src/lib/foo.test.ts")),
    ).toBe(true);
    expect(embeds.some((e) => e.data.title?.startsWith("E2E blocking — smoke"))).toBe(
      true,
    );
  });
});
